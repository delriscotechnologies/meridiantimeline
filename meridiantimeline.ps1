...
$script:Files=New-Object 'System.Collections.Generic.List[object]'; $script:Issues=New-Object 'System.Collections.Generic.List[object]'; $script:OutputPath=''; $script:ChangingZone=$false; $script:SuppressedIssues=0
...
function Pump-Window{$window.Dispatcher.Invoke([Action]{},[Windows.Threading.DispatcherPriority]::Background)}
...
function Normalize-Key([string]$Value){return ($Value-replace'[^a-zA-Z0-9]','').ToLowerInvariant()}
...
function Read-DelimitedFile($Path){
    ...
    try{
        ...
        $profile=Get-ProfileName $headers;$row=1
        while(-not$parser.EndOfData){
            ...
            [void]$result.Add((Add-RowMetadata $record $row '' $profile))
        }
...
function Read-ExcelFile($Path,$Excel){$book=$null;$sheets=$null;$result=New-Object 'System.Collections.Generic.List[object]';try{$book=$Excel.Workbooks.Open($Path,0,$true);$sheets=$book.Worksheets;for($sheetIndex=1;$sheetIndex-le$sheets.Count;$sheetIndex++){$sheet=$null;$used=$null;try{$sheet=$sheets.Item($sheetIndex);$used=$sheet.UsedRange;$rows=[int]$used.Rows.Count;$columns=[int]$used.Columns.Count;if($rows-lt 2-or$columns-lt 2){continue};if($result.Count+$rows-1-gt$MaxRowsPerFile){throw "The workbook exceeds the $MaxRowsPerFile row limit."};$values=$used.Value2;$headers=@();$seen=@{};for($column=1;$column-le$columns;$column++){$base=[string]$values[1,$column];if([string]::IsNullOrWhiteSpace($base)){$base="Column$column"};$name=$base;$suffix=2;while($seen.ContainsKey($name)){$name="$base$suffix";$suffix++};$seen[$name]=$true;$headers+=$name};$profile=Get-ProfileName $headers
                for($row=2;$row-le$rows;$row++){$record=[ordered]@{};$hasValue=$false;for($column=1;$column-le$columns;$column++){$value=$values[$row,$column];$record[$headers[$column-1]]=$value;if($null-ne$value-and-not[string]::IsNullOrWhiteSpace([string]$value)){$hasValue=$true}};if($hasValue){[void]$result.Add((Add-RowMetadata $record ([int]$used.Row+$row-1) ([string]$sheet.Name) $profile))}}
            }finally{if($used){[void][Runtime.InteropServices.Marshal]::ReleaseComObject($used)};if($sheet){[void][Runtime.InteropServices.Marshal]::ReleaseComObject($sheet)}}};return $result.ToArray()}finally{if($sheets){[void][Runtime.InteropServices.Marshal]::ReleaseComObject($sheets)};if($book){$book.Close($false);[void][Runtime.InteropServices.Marshal]::ReleaseComObject($book)}}}
function Get-Names($Profile,$Field){$names=@();$names+=@($profileFields[$Profile][$Field]);if($Profile-ne'Generic'){$names+=@($profileFields['Generic'][$Field])};return @($names|Where-Object{$_}|Select-Object -Unique)}
function Get-RowMap($Row){$map=@{};foreach($property in $Row.PSObject.Properties){if($property.Name-notlike'__Timeline*'){$key=Normalize-Key $property.Name;if(-not$map.ContainsKey($key)){$map[$key]=$property.Value}}};return $map}
function Get-MapValue($Map,[string[]]$Names){foreach($name in $Names){$key=Normalize-Key $name;if($Map.ContainsKey($key)){$value=$Map[$key];if($null-ne$value-and-not[string]::IsNullOrWhiteSpace([string]$value)){return $value}}};return $null}
function Get-MapValues($Map,[string[]]$Names){$values=@();foreach($name in $Names){$key=Normalize-Key $name;if($Map.ContainsKey($key)){$value=[string]$Map[$key];if(-not[string]::IsNullOrWhiteSpace($value)){$values+=$value.Trim()}}};return @($values|Select-Object -Unique)}
function Get-RawLog($Row){$parts=@();foreach($property in $Row.PSObject.Properties){if($property.Name-notlike'__Timeline*'){$value=([string]$property.Value)-replace"`r?`n",' \n ';$parts+="$($property.Name)=$value"}};return ($parts-join' | ')}
...
function Get-Explanation($Event){
    $user=$Event.User;$hostText=if($Event.Host){" on '$($Event.Host)'"}else{''};$ip=if($Event.Ip){" from '$($Event.Ip)'"}else{''}
    if($Event.Profile-eq'Microsoft Entra'){
        $state=if($Event.Outcome-match'(?i)success|succeeded|^0$'){'successful'}elseif($Event.Outcome-match'(?i)fail|denied|interrupt'-or($Event.Code-match'^\d+$'-and$Event.Code-ne'0')){'failed'}else{'recorded'}
        $target=if($Event.Target){" to '$($Event.Target)'"}else{''};$text="Microsoft Entra recorded a $state sign-in for '$user'$target$ip."
        if($state-eq'failed'-and$Event.Message-and$Event.Message-ne$Event.Outcome){$text+=" The source reported: $($Event.Message)."}
        return [pscustomobject]@{Text=(Limit-Text $text 2000);Rule='ENTRA-SIGNIN-v1'}
    }
    if($Event.Profile-eq'Windows Security'){
        $code=([string]$Event.Code-replace'[^0-9]','')
        switch($code){
            ...
            '4688'{$process=if($Event.Process){"'$($Event.Process)'"}else{'an unspecified process'};$text="Windows created $process for '$user'$hostText.";if($Event.Parent){$text+=" Parent process: '$($Event.Parent)'."}}
    ...
    $action=if($Event.Action){" '$($Event.Action)'"}else{' an event'};$outcome=if($Event.Outcome){" Outcome: '$($Event.Outcome)'."}else{''}
...
function Get-TimelineRows($Events){$groups=@{};foreach($event in $Events){$key="$($event.Utc.Ticks)|$($event.Profile)|$($event.User)|$($event.RawLog)";if(-not$groups.ContainsKey($key)){$groups[$key]=[pscustomobject]@{Utc=$event.Utc;First=$event;Items=(New-Object 'System.Collections.Generic.List[object]')}};[void]$groups[$key].Items.Add($event)};$output=New-Object 'System.Collections.Generic.List[object]';$previous=$null;foreach($group in @($groups.Values|Sort-Object Utc,@{Expression={$_.First.Source}})){$items=$group.Items.ToArray();$event=$group.First;$elapsed=if($previous){Format-Interval (($event.Utc-$previous).TotalSeconds)}else{'Start'};$explanation=$event.WhatHappened;if($items.Count-gt 1){$explanation+=" Repeated $($items.Count) times in the imported evidence."};[void]$output.Add([pscustomobject]@{Time=Format-Central $event.Utc;Source=(@($items.Source|Select-Object -Unique)-join'; ');Log=Limit-Text $event.Log;'Time Elapsed Since Last Log'=$elapsed;'What Happened'=Limit-Text $explanation});$previous=$event.Utc};return $output.ToArray()}
function Protect-ExcelValue($Value){if($Value-is[string]-and$Value-match'^[=+\-@]'){return "'$Value"};return $Value}
function Write-Worksheet($Sheet,[string[]]$Headers,$Rows,$TableName){$items=@($Rows);$rowCount=$items.Count+1;$columnCount=$Headers.Count;$data=[Array]::CreateInstance([object],[int[]]@($rowCount,$columnCount));for($column=0;$column-lt$columnCount;$column++){$data.SetValue($Headers[$column],0,$column)};for($row=0;$row-lt$items.Count;$row++){for($column=0;$column-lt$columnCount;$column++){$data.SetValue((Protect-ExcelValue $items[$row].PSObject.Properties[$Headers[$column]].Value),$row+1,$column)}};$range=$Sheet.Range($Sheet.Cells.Item(1,1),$Sheet.Cells.Item($rowCount,$columnCount));$range.NumberFormat='@';$range.Value2=$data;$Sheet.Rows.Item(1).Font.Bold=$true;$Sheet.Rows.Item(1).Interior.Color=0xE8D7C1;$range.WrapText=$true;[void]$range.Columns.AutoFit();for($column=1;$column-le$columnCount;$column++){if($Sheet.Columns.Item($column).ColumnWidth-gt 60){$Sheet.Columns.Item($column).ColumnWidth=60}};if($items.Count){$table=$Sheet.ListObjects.Add(1,$range,$null,1);$table.Name=$TableName;$table.TableStyle='TableStyleMedium2'};$Sheet.Activate();$Sheet.Application.ActiveWindow.SplitRow=1;$Sheet.Application.ActiveWindow.FreezePanes=$true}

if($SelfTest){
    $testPath=Join-Path ([IO.Path]::GetTempPath()) "meridiantimeline_$([guid]::NewGuid().ToString('N')).txt";$testExcel=$null;$testBook=$null;$testSheet=$null
    try{
        [IO.File]::WriteAllText($testPath,"Date`tUser`tApplication`tStatus`tIP address`n2026-08-13T15:00:00Z`tanalyst@contoso.com`tMicrosoft 365`tSuccess`t10.1.2.3",[Text.Encoding]::UTF8)
        $testRows=@(Read-DelimitedFile $testPath);if($testRows.Count-ne 1-or$testRows[0].__TimelineProfile-ne'Microsoft Entra'){throw 'TXT parsing or Entra profile detection failed.'}
        $utc=Convert-Stamp '2026-08-13T15:00:00Z' 'UTC';if((Format-Central $utc)-ne'2026-08-13 10:00:00 CDT'){throw 'UTC to Central conversion failed.'}
        $testEvent=[pscustomobject]@{User='analyst@contoso.com';Host='';Ip='10.1.2.3';Profile='Microsoft Entra';Outcome='Success';Code='0';Target='Microsoft 365';Message='';Process='';Parent=''}
        if((Get-Explanation $testEvent).Text-notmatch'successful sign-in'){throw 'Deterministic explanation failed.'}
        $windowsEvent=[pscustomobject]@{User='analyst';Host='PC-01';Ip='10.1.2.3';Profile='Windows Security';Outcome='Failure';Code='4625';Target='';Message='';Process='';Parent=''};if((Get-Explanation $windowsEvent).Rule-ne'WIN-4625-v1'){throw 'Windows event explanation failed.'}
        $ambiguousRejected=$false;try{[void](Convert-Stamp '2026-11-01 01:30:00' 'Central Standard Time')}catch{$ambiguousRejected=$true};if(-not$ambiguousRejected){throw 'Ambiguous Central timestamp was not rejected.'}
        $duplicateTime=[datetime]'2026-08-13T15:00:00Z';$duplicates=@([pscustomobject]@{Utc=$duplicateTime;Profile='Generic';User='analyst';RawLog='same';Source='a.txt';Log='same';WhatHappened='Test'},[pscustomobject]@{Utc=$duplicateTime;Profile='Generic';User='analyst';RawLog='same';Source='b.txt';Log='same';WhatHappened='Test'});if(@(Get-TimelineRows $duplicates).Count-ne 1){throw 'Exact duplicate grouping failed.'}
        $testExcel=New-Object -ComObject Excel.Application;$testExcel.Visible=$false;$testExcel.DisplayAlerts=$false;$testExcel.AutomationSecurity=3;$testBook=$testExcel.Workbooks.Add();$testSheet=$testBook.Worksheets.Item(1)
        Write-Worksheet $testSheet @('Time','Source','Log','Time Elapsed Since Last Log','What Happened') @([pscustomobject]@{Time='2026-08-13';Source='test.txt';Log='=2+2';'Time Elapsed Since Last Log'='Start';'What Happened'='Test'}) 'SelfTestTimeline'
        if([bool]$testSheet.Cells.Item(2,3).HasFormula){throw 'Excel formula-injection protection failed.'};if([string]$testSheet.Cells.Item(2,3).Text-ne'=2+2'){throw 'Excel-safe text did not preserve the visible log value.'};'SELF_TEST_OK'
    }finally{if($testBook){$testBook.Close($false)};foreach($object in @($testSheet,$testBook)){if($object-and[Runtime.InteropServices.Marshal]::IsComObject($object)){[void][Runtime.InteropServices.Marshal]::ReleaseComObject($object)}};if($testExcel){$testExcel.Quit();[void][Runtime.InteropServices.Marshal]::ReleaseComObject($testExcel)};if(Test-Path -LiteralPath $testPath){Remove-Item -LiteralPath $testPath -Force};[GC]::Collect();[GC]::WaitForPendingFinalizers()}
    exit
}
...
$BuildButton.Add_Click({
    ...
    $BuildButton.IsEnabled=$false;$OpenFileButton.IsEnabled=$false;$OpenFolderButton.IsEnabled=$false;$Progress.Value=2;Set-State Processing 'Building timeline' 'Preparing secure Excel processing.';Pump-Window
    ...
    try{if(-not[type]::GetTypeFromProgID('Excel.Application')){throw 'Microsoft Excel is required to read and create workbooks.'};$excel=New-Object -ComObject Excel.Application;$excel.Visible=$false;$excel.DisplayAlerts=$false;$excel.AskToUpdateLinks=$false;$excel.AutomationSecurity=3;$script:Issues.Clear();$script:SuppressedIssues=0
        ...
        for($fileIndex=0;$fileIndex-lt$script:Files.Count;$fileIndex++){
            $file=$script:Files[$fileIndex];$Progress.Value=5+[int](55*($fileIndex/$script:Files.Count));Set-State Processing 'Reading and normalizing' "$($file.Name)`r`nSource timezone: $($file.ZoneName)";Pump-Window
            ...
            try{
                ...
                foreach($row in $rows){
                    $profile=[string]$row.__TimelineProfile;[void]$profiles.Add($profile);$map=Get-RowMap $row;$stamp=Get-MapValue $map (Get-Names $profile 'Timestamp');$users=@(Get-MapValues $map (Get-Names $profile 'User'))
                    ...
                    $event=[pscustomobject]@{Utc=$utc;OriginalTimestamp=[string]$stamp;User=[string]$user[0];Host=[string](Get-MapValue $map (Get-Names $profile 'Host'));Target=[string](Get-MapValue $map (Get-Names $profile 'Target'));Ip=[string](Get-MapValue $map (Get-Names $profile 'Ip'));Code=[string](Get-MapValue $map (Get-Names $profile 'Code'));Action=[string](Get-MapValue $map (Get-Names $profile 'Action'));Outcome=[string](Get-MapValue $map (Get-Names $profile 'Outcome'));Message=[string](Get-MapValue $map (Get-Names $profile 'Message'));Process=[string](Get-MapValue $map (Get-Names $profile 'Process'));Parent=[string](Get-MapValue $map (Get-Names $profile 'Parent'));Source=$source;Profile=$profile;Zone=$file.ZoneName;Row=$row.__TimelineRow;RawLog=(Limit-Text $raw)}
                    $explanation=Get-Explanation $event;Add-Member -InputObject $event -NotePropertyName WhatHappened -NotePropertyValue $explanation.Text;Add-Member -InputObject $event -NotePropertyName ExplanationRule -NotePropertyValue $explanation.Rule
                    $log=if($event.Message){$event.Message}elseif($event.Action){$event.Action}else{$event.RawLog};Add-Member -InputObject $event -NotePropertyName Log -NotePropertyValue (Limit-Text $log);[void]$events.Add($event)
                }
            ...
            }catch{$file.Status='REVIEW';Add-Issue $file.Name '' $_.Exception.Message ($columns-join', ')}finally{[void]$sourceFiles.Add([pscustomobject]@{File=$file.Name;'SHA-256'=$hash;'Size MB'=$sizeMb;'Source Timezone'=$file.ZoneName;'Detected Profile'=$profileNames;'Rows Read'=$rowsRead;'Matching Events'=($events.Count-$matchedBefore);Status=$status});Update-Files $fileIndex;Pump-Window}
        }
        ...
        $evidence=New-Object 'System.Collections.Generic.List[object]';$previous=$null;foreach($event in $ordered){$delta=if($previous){Format-Interval(($event.Utc-$previous).TotalSeconds)}else{'Start'};[void]$evidence.Add([pscustomobject]@{'Original Timestamp'=$event.OriginalTimestamp;'Central Time'=Format-Central $event.Utc;'UTC Time'=$event.Utc.ToString('yyyy-MM-dd HH:mm:ss')+' UTC';'Time Since Previous'=$delta;'Observed User'=$event.User;'Source Profile'=$event.Profile;'Source Host'=$event.Host;Target=$event.Target;'Source IP'=$event.Ip;'Event Code'=$event.Code;Action=$event.Action;Outcome=$event.Outcome;Log=$event.Log;'What Happened'=$event.WhatHappened;'Explanation Rule'=$event.ExplanationRule;Source=$event.Source;'Source Timezone'=$event.Zone;'Source Row'=$event.Row;'Raw Event'=$event.RawLog});$previous=$event.Utc}
        $Progress.Value=75;Set-State Processing 'Creating workbook' 'Writing Timeline, Evidence, Source Files, and Import Issues.';Pump-Window;$book=$excel.Workbooks.Add();while($book.Worksheets.Count-gt 1){$book.Worksheets.Item($book.Worksheets.Count).Delete()};$timelineSheet=$book.Worksheets.Item(1);$timelineSheet.Name='Timeline';Write-Worksheet $timelineSheet @('Time','Source','Log','Time Elapsed Since Last Log','What Happened') $timeline 'InvestigationTimeline';$evidenceSheet=$book.Worksheets.Add();$evidenceSheet.Name='Evidence';Write-Worksheet $evidenceSheet @('Original Timestamp','Central Time','UTC Time','Time Since Previous','Observed User','Source Profile','Source Host','Target','Source IP','Event Code','Action','Outcome','Log','What Happened','Explanation Rule','Source','Source Timezone','Source Row','Raw Event') $evidence.ToArray() 'InvestigationEvidence';$sourceSheet=$book.Worksheets.Add();$sourceSheet.Name='Source Files';Write-Worksheet $sourceSheet @('File','SHA-256','Size MB','Source Timezone','Detected Profile','Rows Read','Matching Events','Status') $sourceFiles.ToArray() 'InvestigationSources';$issueSheet=$book.Worksheets.Add();$issueSheet.Name='Import Issues';Write-Worksheet $issueSheet @('File','Row','Issue','Detected Columns') $script:Issues.ToArray() 'TimelineImportIssues';$timelineSheet.Move($book.Worksheets.Item(1));$timelineSheet.Activate();$book.SaveAs($save.FileName,51);$script:OutputPath=$save.FileName;$Progress.Value=100;$OpenFileButton.IsEnabled=$true;$OpenFolderButton.IsEnabled=$true
    ...
    }catch{$Progress.Value=0;Set-State Error 'Timeline could not be created' $_.Exception.Message}finally{if($book){$book.Close($false)};foreach($object in @($issueSheet,$sourceSheet,$evidenceSheet,$timelineSheet,$book)){if($object-and[Runtime.InteropServices.Marshal]::IsComObject($object)){[void][Runtime.InteropServices.Marshal]::ReleaseComObject($object)}};if($excel){$excel.Quit();[void][Runtime.InteropServices.Marshal]::ReleaseComObject($excel)};[GC]::Collect();[GC]::WaitForPendingFinalizers();$BuildButton.IsEnabled=$true}
})
