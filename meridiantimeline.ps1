...
function ConvertTo-NormalizedKey([string]$Value){return ($Value-replace'[^a-zA-Z0-9]','').ToLowerInvariant()}
...
function Get-Names($ProfileName,$Field){$names=@();$names+=@($profileFields[$ProfileName][$Field]);if($ProfileName-ne'Generic'){$names+=@($profileFields['Generic'][$Field])};return @($names|Where-Object{$_}|Select-Object -Unique)}
...
function Get-MapValue($Map,[string[]]$Names){foreach($name in $Names){$key=ConvertTo-NormalizedKey $name;if($Map.ContainsKey($key)){$value=$Map[$key];if($null-ne$value-and-not[string]::IsNullOrWhiteSpace([string]$value)){return $value}}};return $null}
function Get-MapValues($Map,[string[]]$Names){$values=@();foreach($name in $Names){$key=ConvertTo-NormalizedKey $name;if($Map.ContainsKey($key)){$value=[string]$Map[$key];if(-not[string]::IsNullOrWhiteSpace($value)){$values+=$value.Trim()}}};return @($values|Select-Object -Unique)}
    ...
    try{if(-not[type]::GetTypeFromProgID('Excel.Application')){throw 'Microsoft Excel is required to read and create workbooks.'};$excel=New-Object -ComObject Excel.Application;$excel.Visible=$false;$excel.DisplayAlerts=$false;$excel.AskToUpdateLinks=$false;$excel.AutomationSecurity=3;$script:Issues.Clear();$script:SuppressedIssues=0
        ...
        for($fileIndex=0;$fileIndex-lt$script:Files.Count;$fileIndex++){
            ...
            try{
                ...
                foreach($row in $rows){
                    $ProfileName=[string]$row.__TimelineProfile;[void]$profiles.Add($ProfileName);$map=Get-RowMap $row;$stamp=Get-MapValue $map (Get-Names $ProfileName 'Timestamp');$users=@(Get-MapValues $map (Get-Names $ProfileName 'User'))
                    ...
                    $TimelineEvent=[pscustomobject]@{Utc=$utc;OriginalTimestamp=[string]$stamp;User=[string]$user[0];Host=[string](Get-MapValue $map (Get-Names $ProfileName 'Host'));Target=[string](Get-MapValue $map (Get-Names $ProfileName 'Target'));Ip=[string](Get-MapValue $map (Get-Names $ProfileName 'Ip'));Code=[string](Get-MapValue $map (Get-Names $ProfileName 'Code'));Action=[string](Get-MapValue $map (Get-Names $ProfileName 'Action'));Outcome=[string](Get-MapValue $map (Get-Names $ProfileName 'Outcome'));Message=[string](Get-MapValue $map (Get-Names $ProfileName 'Message'));Process=[string](Get-MapValue $map (Get-Names $ProfileName 'Process'));Parent=[string](Get-MapValue $map (Get-Names $ProfileName 'Parent'));Source=$source;Profile=$ProfileName;Zone=$file.ZoneName;Row=$row.__TimelineRow;RawLog=(Limit-Text $raw)}
                    $explanation=Get-Explanation $TimelineEvent;Add-Member -InputObject $TimelineEvent -NotePropertyName WhatHappened -NotePropertyValue $explanation.Text;Add-Member -InputObject $TimelineEvent -NotePropertyName ExplanationRule -NotePropertyValue $explanation.Rule
                    $log=if($TimelineEvent.Message){$TimelineEvent.Message}elseif($TimelineEvent.Action){$TimelineEvent.Action}else{$TimelineEvent.RawLog};Add-Member -InputObject $TimelineEvent -NotePropertyName Log -NotePropertyValue (Limit-Text $log);[void]$events.Add($TimelineEvent)
                }
